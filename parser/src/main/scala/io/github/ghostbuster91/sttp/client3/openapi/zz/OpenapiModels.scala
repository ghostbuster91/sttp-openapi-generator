...
object OpenapiModels {
  ...
  implicit val OpenapiResponseContentDecoder
      : Decoder[Seq[OpenapiResponseContent]] = { (c: HCursor) =>
    ...
    } yield responses.map { case (ct, s) =>
      OpenapiResponseContent(ct, s.d)
    }.toSeq
  }

  implicit val OpenapiResponseDecoder
      : Decoder[Seq[OpenapiResponse]] = { (c: HCursor) =>
    implicit val InnerDecoder
        : Decoder[(String, Seq[OpenapiResponseContent])] = { (c: HCursor) =>
      for {
    ...
    } yield schema.map { case (code, (desc, content)) =>
      OpenapiResponse(code, desc, content)
    }.toSeq
  }

  implicit val OpenapiRequestBodyContentDecoder
      : Decoder[Seq[OpenapiRequestBodyContent]] = { (c: HCursor) =>
    case class Holder(d: OpenapiSchemaType)
    implicit val InnerDecoder: Decoder[Holder] = { (c: HCursor) =>
      for {
        schema <- c.downField("schema").as[OpenapiSchemaType]
    ...
    for {
      ...
      OpenapiRequestBodyContent(ct, s.d)
    }.toSeq
  }

  implicit val OpenapiRequestBodyDecoder
      : Decoder[OpenapiRequestBody] = { (c: HCursor) =>
    implicit val InnerDecoder
        : Decoder[(String, Seq[OpenapiRequestBodyContent])] = { (c: HCursor) =>
      for {
        description <- c.downField("description").as[String]
    ...
    for {
      ...
      content <- c.downField("content").as[Seq[OpenapiRequestBodyContent]]
    } yield OpenapiRequestBody(
      required = requiredOpt.getOrElse(false),
      description,
      content
    )
  }

  implicit val OpenapiInfoDecoder: Decoder[OpenapiInfo] =
    deriveDecoder[OpenapiInfo]
  implicit val OpenapiParameterDecoder: Decoder[OpenapiParameter] =
    deriveDecoder[OpenapiParameter]
  implicit val OpenapiPathMethodDecoder
      : Decoder[Seq[OpenapiPathMethod]] = { (c: HCursor) =>
    implicit val InnerDecoder: Decoder[
      ...
      } yield (parameters, responses, requestBody, summary, tags)
    }
    for {
      methods <- c.as[
        Map[
    ...
    } yield methods.map { case (t, (p, r, rb, s, tg)) =>
      OpenapiPathMethod(t, p, r, rb, s, tg)
    }.toSeq
  }
  ...
  implicit val OpenapiPathDecoder: Decoder[Seq[OpenapiPath]] = { (c: HCursor) =>
    ...
    } yield paths.map { case (url, ms) => OpenapiPath(url, ms) }.toSeq
  }

  implicit val OpenapiDocumentDecoder: Decoder[OpenapiDocument] =
    deriveDecoder[OpenapiDocument]
