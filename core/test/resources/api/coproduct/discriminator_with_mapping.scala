...
trait CirceCodecs extends AutoDerivation with SttpCirceApi {
  ...
  implicit val entityEncoder: Encoder[Entity] = new Encoder[Entity] {
    override def apply(entity: Entity): Json = entity match {
      case person: Person =>
        Json
          .obj("name" -> Encoder[String].apply("john"))
          .deepMerge(Encoder[Person].apply(person))
      case organization: Organization =>
        Json
          .obj("name" -> Encoder[String].apply("sml"))
          .deepMerge(Encoder[Organization].apply(organization))
    }
  }
}
